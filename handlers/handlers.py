...
from databases.requests import get_product
...
async def category(message: Message):
    await message.answer('Выберите категорию телефона', reply_markup=await KB.categories())

@router.callback_query(F.data.startswith('category_'))
async def category_select(callback: CallbackQuery):
    category_id = callback.data.split('_')[1]
    await callback.message.answer(f'Товары по этой катеогрии ', reply_markup=await KB.product(category_id=category_id))
